...
// file, the SQLite one build-tagged, so a binary built without it (-tags nosqlite)
// does not link SQLite at all and refuses that driver at startup.
package persistence
...
func Open(cfg *config.Config, logger *slog.Logger) (*Store, func(), error) {
	...
	case database.DriverPostgres:
		store, closeFn, err = openPostgres(cfg, logger)
	default:
