...
func mapType(expr ast.Expr) (tsType, dep string, sh fieldShape, err error) {
	...
	case *ast.MapType:
		// map[string]any -> Record<string, unknown> — the open params bag of
		// ApiMessage. Only this one map shape is mapped; a map with typed
		// values would silently weaken to unknown, so it stays an error.
		key, isKeyIdent := t.Key.(*ast.Ident)
		value, isValueIdent := t.Value.(*ast.Ident)
		if isKeyIdent && isValueIdent && key.Name == "string" &&
			(value.Name == "any" || value.Name == "interface{}") {
			return "Record<string, unknown>", "", fieldShape{kind: "record"}, nil
		}
		return "", "", fieldShape{}, fmt.Errorf(
			"unmapped Go type %T — only map[string]any is supported (Record<string, unknown>)",
			expr)
	case *ast.ArrayType:
...
// artifacts adapts the rendered files to the shared write/check helpers.
func artifacts(files map[string]string) map[string][]byte {
...
// writeFiles writes the rendered files and prunes the orphans — the pruning is
// tsgen's own concern (a deleted //gkts directive leaves a now-authorless file
// behind), the writing is the shared one.
func writeFiles(root string, files map[string]string) {
	if err := tool.WriteArtifacts("tsgen", root, artifacts(files)); err != nil {
		fatal("%v", err)
	}
...
func checkFiles(root string, files map[string]string) {
	drift := tool.CheckArtifacts(root, artifacts(files), "make ts-gen")
	for _, rel := range orphans(root, files) {
		drift = append(drift, fmt.Sprintf(
			"%s is an orphan — no //gkts directive produces it; run `make ts-gen` to prune it",
			rel))
	}
	if len(drift) > 0 {
		sort.Strings(drift)
