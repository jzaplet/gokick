...
- Requests:
  ...
  - **Data endpoints require `validate`.** When `Data ≠ null`, options must carry `validate:` — the tsgen-generated guard (`isAdminUser`, `arrayOf(isAdminUser)`, `isLoginResponse`, …) — or the call does not compile; the 2xx body is then checked against the generated contract at runtime. A contract-violating body becomes a transport failure and reports to Sentry. 204 endpoints (`Data = null`) take no guard.
  - **Failure narrowing.** `ApiResponse` failures are `ApiError<TErrors>` (API error body) **or** `ApiTransport` (network / malformed body / contract violation — top-level `message`, no `data`). Reading `result.data` on a failure does not compile until narrowed: `errors.value = isTransport(result) ? { general: result.message } : result.data;`.
