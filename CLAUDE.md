...
make test-pg                                     # The Go suite on Postgres: db-test container, -tags nosqlite (or APP_TEST_DB_URL=<superuser DSN>)
...
**PlaneMiddleware** (in `BaseChain`, before Tenant) marks the `shared.Plane` in ctx from the declared permission — `platform:*` → platform, anything else → tenant (the zero value, fail closed); the `SystemCommandBus` marks its commands `system`, and so does a pre-tenant command (`shared.PreTenant` — login and refresh only, an allow-list gate: they find the account before any tenant is known). The adapter opens a transaction on the plane's role: on Postgres the tenant plane runs as `gokick_app` (row-level security, scoped to the tenant by a tx-local setting), platform/system as `gokick_system` (BYPASSRLS); on SQLite the plane changes nothing. **ReadTxMiddleware** closes the QueryBus chain: `Transactor.BeginReadTx` wraps each query in a READ ONLY transaction on Postgres (the tenant scope is transaction-local, so an unscoped read would see no row) and is a no-op on SQLite.
...
**TransactionMiddleware retries a transaction that lost a race.** On Postgres a deadlock (40P01), a serialization failure (40001) or a lock wait past `lock_timeout` (55P03) — `shared.Transactor.IsRetryable`, always false on SQLite — rolls the attempt back and reruns the whole handler in a fresh transaction, at most 3 attempts with a jittered pause. Each attempt collects events and audit records into collectors of its own; only the last attempt's reach DispatchEvents / Audit (events only on commit), so a retried command dispatches and audits once. A handler must therefore tolerate a rerun: writes through `Conn(ctx)` roll back with the attempt, anything outside the transaction would repeat (the raw-pool commands are `SkipsTransaction`, and skip the retry too).
...
| `postgres/` | The Postgres adapter: `Manager` (two pgx pools — `gokick_app` for the tenant plane, `gokick_system` BYPASSRLS for platform/system; `BeginTx`/`BeginReadTx` pick the pool by `shared.Plane` and scope a tenant-plane tx with `set_config('app.tenant_id', …, true)`; session `lock_timeout`/`statement_timeout`/`idle_in_transaction_session_timeout`), `Migrator` (goose over `migrations/postgres` as `gokick_owner`, advisory-locked, then `VerifyRoles` refuses a superuser/BYPASSRLS/owner runtime role, or a member of one), `BaseRepository` (`r.Conn(ctx)`: the tx in ctx, else the system pool on a cross-tenant plane, else a per-statement tx scoped to ctx's tenant; `r.SystemConn(ctx)` for cross-tenant work — `*AcrossTenants`, worker bookkeeping, the nickname lookup; `r.SystemTx(ctx, fn)` for cross-tenant work of several statements that must share one transaction, e.g. lock rows then act; `r.DB.System()` = the raw-pool twin), `Locker` (`shared.Locker` on session advisory locks, all on one dedicated system-pool connection — the scheduler's per-job lock), `LockInIDOrder` (bulk writes lock their rows in id order), `ParseID` (a malformed id is a missing row, as on SQLite), `Args` ($n builder), `NowExpr`/`NowPlus` (`statement_timestamp()`, never `now()`). Tests: `make test-pg` (template-clone harness `app/internal/testfx/pgfx`, RLS suite `rls_test.go`) |
| `postgres/user\|tenant\|token\|run\|audit` | The Postgres twins of the SQLite repositories — same port contracts; `ClaimDue` claims `FOR UPDATE SKIP LOCKED`; grid text filters `ILIKE` under `app_sort`; unique violations map to the handlers' field errors; a tenant delete locks the tenant rows before deciding they are empty; a user edit (`Update`) writes only the columns it owns (never `active`, the password only when set) and `UpdatePassword` swaps only the hash the old password was verified against, so neither undoes a concurrent write |
| `persistence/` | `Open(cfg)` → `Store` (every database port: repositories, `Tx` Transactor, `Audit`, `Locker`, `Migrator`) + cleanup that closes the pool — the single place that knows which adapter backs the ports. Branches on `APP_DB_DRIVER`; each adapter's opener is its own file (`sqlite.go` is `//go:build !nosqlite`, `postgres.go` always built) |
...
wire.FieldsOf(new(*persistence.Store), "Users", "PlatformUsers", "Tokens", "Runs",
    "Tenants", "PlatformTenants", "Audit", "Tx", "Locker", "Migrator"),
...
2. **`infrastructure/sqlite/<context>/`** + **`infrastructure/postgres/<context>/`** — repository implementation, one per adapter (the contract tests in `app/internal/repotest/<context>` run on both)
...
Broad-glob components auto-cover new sub-packages: a new `application/<ctx>/command/` matches `application/**`, a new handler matches `presentation/http/handler/**`. But the **bounded-context** components are enumerated, not wildcarded — `domain` is split per context (`domain_user`, `domain_token`, `domain_run`, `domain_tenant`) and `sqlite_repos` / `postgres_repos` list each repo dir — exactly so a cross-context import is caught. So adding a new context (`domain/order/`, `infrastructure/sqlite/order/`) **does** require editing `.go-arch-lint.yml`: add a `domain_order` component, grant it in each consumer's `mayDependOn` (`application`, `sqlite_repos`, `postgres_repos`, `testfx`, `repotest`, …), and add `infrastructure/sqlite/order/**` to `sqlite_repos` and `infrastructure/postgres/order/**` to `postgres_repos`. That edit is the price of enforcing cross-context isolation in the linter.
...
- **`r.Conn(ctx)` in repositories.** Always use `r.Conn(ctx)` (from embedded `BaseRepository`), never `r.DB.DB()` directly. This ensures transparent transaction participation. **Exception:** writes that MUST commit independently of any bus tx — `user.Repository.RecordFailedLogin/ResetFailedLogin/RecordLogin` (login is `SkipTransaction`, so these single-statement writes auto-commit; the raw pool also future-proofs against an in-tx caller) and `audit.Repository.Save` (must survive the business rollback) — use `r.DB.DB()` on purpose. These are the only legitimate raw-pool callers; document the reason in the method comment. On Postgres the raw pool is `r.DB.System()` (the system role), and `r.SystemConn(ctx)` / `r.SystemTx(ctx, fn)` run cross-tenant work on that role too — every method calling one of them is on an allow-list gate (`app/zz_pgsystem_test.go`).
- **Tenant scoping (multitenancy).** Every SQL query on a tenant-owned table (`users`, …) must scope by `tenant_id` (from `r.Tenant(ctx)`) OR carry an inline `/* tenant-scope-exempt: <reason> */` marker — the `app/zz_tenant_test.go` conformance gate (both adapters) fails CI otherwise (an unclassified table also fails). The resolver supplies the tenant, the repo applies it (no transparent `WHERE` injection); `r.Tenant(ctx)` panics on a missing tenant in multitenant mode (fail-closed). Cross-tenant platform reads carry `tenant-scope-exempt: platform superadmin`. See `/gk-multitenancy`.
