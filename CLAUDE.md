...
- **Structured log attributes.** The standardized correlation/measurement vocabulary comes from `shared.LogAttrs(ctx)` (`trace_id` + `user_id`), `shared.DurationMsAttr` (numeric `duration_ms`) and the `shared.LogKey*` constants, used by the bus middleware where consistency matters most. Leaf component logs (worker, scheduler, server, CLI) and one-off component-local keys may use plain string literals. The `*slog.Logger` is constructed once in `cmd/logger.go` — the single seam where an OpenTelemetry handler would later wrap it. See [Observability](docs/framework/infrastructure/observability.md).
