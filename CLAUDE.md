...
- **Wire payloads are gkts-annotated named structs.** Every `Responder.JSON` payload and `request.DecodeJSON` target must be a named struct carrying a `//gkts:<path> <TSName>` directive (tsgen generates the TS side; `make ts-gen` after changing a DTO). `make boundary-check` (in `make lint`) fails on inline maps, `any`, or unannotated types; the only escape is a call-site `//gkts:ignore <reason>` for endpoints the SPA never consumes (`/health`, `APP_RUN_DEBUG` debug endpoints).
