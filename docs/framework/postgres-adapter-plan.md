...
**Status:** 🟢 plán schválen — všechna rozhodnutí D1–D10 potvrzena · fáze 0–5 hotové (fáze 5: zámky a souběh — opakování commandu po prohraném souběhu, zámek jobu scheduleru, zamykání při mazání tenanta, hromadných zápisech a editaci uživatele; odchylky od plánu v sekci 9, „Fáze 3/4/5 — co dopadlo jinak") · **Roadmap:** [Roadmap (GoKick) → Škálovatelnost](/framework/gokick-roadmap) · **Analýza:** 2026-09-24
...
1. **Jeden přepínač, jedno místo volby.** Backend vybírá `APP_DB_DRIVER=sqlite|postgres` (plus DSN proměnné). Větví se jen `persistence.Open(cfg)` (volá ji Wire) a jeho zrcadlo v `testfx`, které čte **stejnou** proměnnou a sestaví Store **stejnou** funkcí adaptéru (`persistence.SQLiteStore` / `persistence.PostgresStore`). Testy tak z principu jedou na backendu, který určuje `APP_DB_DRIVER`, se stejným zapojením jako aplikace.
...
| N14 | Scheduler běží v každé `serve` replice; test `TestScheduler_TwoInstancesTickIndependently` to výslovně dokumentoval. | Každý job proběhne N× za interval. | Advisory lock pro každý job (sekce 5). ✅ **Fáze 5:** zámek jobu drží jedna replika, dokud běží. |
...
| N16 | Edit uživatele (admin i platformní) načte řádek a zapíše ho zpět celý, včetně `active` a `password_hash`. Na SQLite mezi čtení a zápis nic nevstoupí, protože zápisová transakce drží zámek celé DB. Nález z fáze 5, ověřený mutační kontrolou na Postgresu. | Pod READ COMMITTED se souběžná deaktivace nebo změna hesla, která se commitne mezi čtením a zápisem editu, tiše přepíše: účet je znovu aktivní, staré heslo znovu platí. | ✅ **Fáze 5:** edit zapisuje jen své sloupce: `Update` nepíše `active` a heslo jen nové. Souběžný zápis zůstane. Stejnou chybu měla změna hesla: staré heslo ověřila proti přečtenému hashi a nové zapsala bez podmínky. Teď je to compare-and-swap na ověřený hash. |
...
| Smazání tenanta vs. souběžný insert uživatele nebo runu | serializace | FK na obou stranách (N4); FK check drží `KEY SHARE` na řádku tenanta. ✅ **Fáze 5:** mazání nejdřív zamkne řádky tenantů a o prázdnotě rozhodne až další příkaz s čerstvým snapshotem (viz „Fáze 5 — co dopadlo jinak"). |
| Invarianty přes více řádků (guardy superadmina, budoucí „poslední admin") | serializace | zámek na „kotvě": `SELECT … FROM tenants WHERE id=$1 FOR UPDATE`, případně `pg_advisory_xact_lock(key)`; alternativně `SERIALIZABLE` (viz níže). Ve fázi 5 žádný takový invariant neexistuje. |
| Edit, který načtený řádek zapíše celý zpět | serializace | ✅ **Fáze 5:** edit zapisuje jen sloupce, které mění; zápis závislý na přečtené hodnotě je compare-and-swap (N16) |
| Bulk operace nad mnoha řádky | serializace | zamykat v deterministickém pořadí (`… WHERE id IN (SELECT id … ORDER BY id FOR UPDATE)`); 40P01 (deadlock) je retryovatelný. ✅ **Fáze 5:** `postgres.LockInIDOrder` |
| Scheduler v N replikách | každá replika spustí job | ✅ **Fáze 5:** `Locker.Hold("job:<name>")` před každým během: session `pg_try_advisory_lock` na vyhrazeném spojení. Replika zámek drží, dokud běží, ne jen po dobu jobu. Nevlastník tick přeskočí. |
...
| Čekání na zámek | `busy_timeout(5000)` | `lock_timeout=5s` → chyba 55P03 se namapuje na retryovatelnou chybu. ✅ **Fáze 5:** bus command zopakuje. |
...
✅ **Fáze 5:** `TransactionMiddleware` opakuje každý command, jehož transakce prohraje souběh (40001, 40P01 i 55P03), nejvýš 3×. Marker `RequiresSerializable` nevznikl, protože ho žádný dnešní invariant nepotřebuje.
...
✅ **Fáze 4** (`app/infrastructure/postgres/conn.go`): `BaseRepository.Conn(ctx)` vrátí transakci z ctx; bez ní na platformní a systémové rovině systémový pool a na tenantové krátkou transakci na jeden příkaz, scopnutou na tenanta z ctx (event a run handlery; chybějící tenant v multitenant módu = chyba). `SystemConn(ctx)` běží na systémové roli bez ohledu na rovinu a k transakci z ctx se připojí, jen když je cross-tenant. Používají ho metody `*AcrossTenants` (jejich kontrakt je sáhnout do všech tenantů, jako na SQLite), bookkeeping workeru, sweep tokenů a `FindByNickname`; `r.DB.System()` je dvojče raw poolu (čítače loginu, audit). Místo markerů `(rls)`/`(system)` drží gate `app/zz_pgsystem_test.go` allow-list metod, které na systémovou roli sahají. **Login a refresh** běží v systémové rovině (`shared.PreTenant`, allow-list `app/application/zz_pretenant_test.go`), viz „Fáze 4 — co dopadlo jinak".
...
| Gate | Změna |
|---|---|
| `app/zz_tenant_test.go` (dřív v `infrastructure/sqlite`) | ✅ **Fáze 4:** přesunut na úroveň `app/` a parametrizovaný adaptérem — kontroluje SQL `infrastructure/sqlite` i `infrastructure/postgres` a běží v každém běhu (čte jen zdroj); `tableRe` ignoruje `FOR UPDATE/SHARE`, `DO UPDATE` a jména CTE, systémové katalogy (`pg_roles`, …) jsou výjimkou jen pro Postgres. Původní plán: **Guard proti prázdnému výsledku** ✅ (fáze 2): každý balíček repozitáře musí dát aspoň jeden dotaz a write gate aspoň jeden INSERT s `tenant_id`, jinak test selže jako „scan went blind". Pro PG SQL se opraví `tableRe`, který by jinak chytal `FOR UPDATE SKIP LOCKED` jako tabulku `skip`, podobně `ON CONFLICT … DO UPDATE SET`, `EXTRACT(… FROM …)` a `FROM unnest(…)`. Exempt tabulky budou pro každý dialekt zvlášť. Markery dostanou rovinu `(rls)` nebo `(system)` (6.1). |
| `app/zz_sqltime_test.go` (dřív v `infrastructure/sqlite`) | Zůstane jen pro SQLite (tag `!nosqlite`); guard proti prázdnému výsledku ✅ (fáze 2). ✅ **Fáze 4:** PG dvojče `app/zz_pgtime_test.go` zakáže `now()`, `CURRENT_TIMESTAMP`, `transaction_timestamp()` a `LOCALTIMESTAMP` v SQL adaptéru i v PG migracích (N9); hodiny jsou `statement_timestamp()`. |
| ✅ **nový** `app/zz_pgsystem_test.go` (fáze 4) | Allow-list metod Postgres repozitářů, které volají `SystemConn` nebo `r.DB.System()` (systémová role obchází RLS); `*AcrossTenants` jsou povolené jménem. Hlídá i zastaralé položky. |
...
- **Dva paralelní joby, oba povinné:**
  ...
  ✅ **Částečně už ve fázi 3:** job `postgres tests` (`make test-pg`: `db-test` ze stejného Dockerfile, IP kontejneru, `-tags nosqlite`) běží od fáze 3. Od fáze 4 pouští celou Go suite; povinnost v rulesetu zůstává ve fázi 6.
...
| **4 — PG repozitáře** ✅ | `user`, `tenant`, `token`, `run`, `audit` v `$n` SQL; `ClaimDue` se `SKIP LOCKED`; časy přes `statement_timestamp()`; `ILIKE` s escapováním; `NULLS FIRST/LAST`; parse UUID; mapování chyb (23505, 23503, 42501, 40001, 40P01, 55P03, 22P02). `zz_tenant` pro PG. | ✅ hotovo: celá Go suite zelená na SQLite i na Postgresu (`make test-pg` = `./app/... ./cmd/...` s `-tags nosqlite`), `persistence.Open` Postgres otevře |
| **5 — zámky** ✅ | Advisory `Locker` pro scheduler, locker migrací, timeouty (sekce 3.2), zamykání „kotvy" u invariantů, deterministické pořadí u bulk operací, volitelný `RequiresSerializable` + retry. PG testy souběhu (N workerů × M runů exactly-once, dvě instance scheduleru, souběžné migrace). Úprava `TestScheduler_TwoInstancesTickIndependently`. | ✅ hotovo: testy souběhu zelené na PG 18 i na SQLite, na PG 20× po sobě s `-race` |
...
- **Adaptér zatím není volitelný:** `persistence.Open` pro `postgres` vrací „not available yet" a `make build && make serve` s `APP_DB_DRIVER=postgres` sice nahodí databázi, ale aplikace start odmítne. `make test-pg` pokrývá balíčky adaptéru (`PG_TEST_PKGS`); ve fázi 4 se rozšíří na celou suite. (Fáze 4 obojí uzavřela.)
...
### Fáze 4 — co dopadlo jinak

- **Login a refresh v systémové rovině (`shared.PreTenant`).** Plán počítal s tím, že o systémové rovině rozhodne metoda repozitáře. U loginu a refreshe to nestačí: přicházejí bez claims, `TenantMiddleware` jim dá výchozího tenanta a účet i jeho tokeny můžou být v kterémkoli tenantu. Uložení tokenu, `users.FindByID` v refreshi a hlavně `DeleteByUserID` při detekci krádeže by pod RLS výchozího tenanta tiše nic nenašly (u krádeže = neprovedený force-logout). Oba commandy proto nesou marker `shared.PreTenant` a `PlaneMiddleware` jim (jen spolu se `SkipPermission`) dá systémovou rovinu. Implementátoři jsou na allow-listu (`app/application/zz_pretenant_test.go`); test přes bus s uživatelem jiného tenanta (`app/application/auth/command/pretenant_test.go`) na Postgresu bez markeru padá.
- **`*AcrossTenants` přes `SystemConn`, nezávisle na rovině.** Kontrakt platformního portu je sáhnout do všech tenantů a SQLite ctx rovinu ignoruje; aby port dělal totéž na obou DB, jdou tyto metody na systémovou roli vždy. Kdo je smí volat, hlídá dál `zz_platform_isolation` a samostatný port.
- **Místo markerů `(rls)`/`(system)` allow-list** (`app/zz_pgsystem_test.go`): gate hlídá metody, které na systémovou roli skutečně sahají (`SystemConn`, `r.DB.System()`), což je přesnější než klasifikace komentářů.
- **Gaty přesunuty na úroveň `app/`:** tenant conformance kontroluje oba adaptéry v každém běhu; SQLite gate na datumy leží vedle svého PG dvojčete.
- **Mapování chyb:** porušení unikátnosti nicku a jména tenanta vrací na **obou** adaptérech tutéž `ValidationError` jako pre-check handleru (SQLite podle kódu `CONSTRAINT_UNIQUE`, Postgres podle jména constraintu) — kontraktní test `app/internal/repotest/user/constraint_test.go`. 23503 u mazání tenanta = „tenant není prázdný" (odmítnutí; ve fázi 5 ho nahradil zámek, viz níže). 22P02 nenastane: id z venku projde `postgres.ParseID`, nevalidní UUID je neexistující řádek. 42501 zůstává interní chybou (je to bug). 40001/40P01/55P03 a retry patří k zámkům do fáze 5.
- **Testy a roviny:** holý `context.Background()` je tenantová rovina výchozího tenanta. Testy, které volají handler nebo repozitář bez busu, dostaly ctx, jaký by jim dal bus: `testfx.TenantCtx`, `testfx.PlatformCtx`, `testfx.SystemCtx` (inspekce řádku cizího tenanta). Testy configu smažou zděděné `APP_*` proměnné, aby do nich nepronikl `APP_DB_DRIVER` běhu.
- **Čas:** `timestamptz` se čte v UTC (pgx codec na každém spojení), takže časy se vrací stejně jako ze SQLite.
- **Cena mimo bus:** příkaz na tenantové rovině bez transakce stojí `BEGIN` + `set_config` + `COMMIT` navíc; týká se jen event a run handlerů.
- **Zbývá:** e2e na Postgresu a `make migrate-*` pro Postgres (fáze 6 / později), zámky scheduleru a retry (fáze 5 ✅).
- **Ověřeno:** celá suite na PostgreSQL 16 lokálně (PG 18 ověří CI job) i na SQLite; `-race` nad balíčky adaptéru, repotest, workeru a auth; nové testy mají mutační kontrolu (scope bez transakce, `SystemConn` v tenantové/systémové transakci, `PreTenant` na loginu i refreshi, mapování unikátnosti, `ParseID`, tenant gate nad PG SQL). Binárka s `-tags nosqlite` nad Postgresem v multitenant módu: `seed` (tenant, admin, superadmin, audit), `create-user`, `serve`; admin tenanta Acme se přihlásí přes HTTP a jeho grid ukáže jen uživatele Acme, superadminův platformní grid všechny tři napříč tenanty; v logu žádná chyba.

### Fáze 5 — co dopadlo jinak

- **Zámek jobu drží replika, dokud běží.** Plán počítal se zámkem jen na dobu jobu („job, pak `pg_advisory_unlock`"). To by zabránilo jen překryvu: repliky tikají v různých okamžicích, takže každá by job dál pouštěla jednou za interval a N14 by zůstal. Replika proto zámek drží a uvolní ho, až scheduler zastaví. Když spadne, zámek zahodí databáze. Port je `shared.Locker` s metodami `Hold` a `Release` (ne `TryWithLock`). Všechny zámky jedné repliky leží na jednom vyhrazeném spojení systémového poolu a před každým během se ověří v `pg_locks`. SQLite obsluhuje jeden proces, a proto jeho locker povolí každý zámek.
- **Opakování bez `RequiresSerializable`.** `TransactionMiddleware` opakuje každý command, jehož transakce prohraje souboj se souběžnou transakcí (40001, 40P01, 55P03; rozhoduje `Transactor.IsRetryable`). Pokusy jsou nejvýš tři, mezi nimi je náhodná pauza. Každý pokus má vlastní sběrače eventů a auditu a ven jdou jen záznamy posledního pokusu. Pevný časový strop opakování nemá: `lock_timeout` omezuje každé jednotlivé čekání na zámek, ne celý pokus. Command za dlouhým držitelem zámku tak může trvat až třikrát déle. Write timeout HTTP serveru ho neukončí, jen zahodí odpověď.
- **Mazání tenanta: zámek a dva příkazy.** Jeden příkaz nestačí, ani CTE s `FOR UPDATE`: `NOT EXISTS` vidí snapshot příkazu, a ne insert commitnutý během čekání na zámek. Mazání proto nejdřív zamkne tenanty v pořadí podle id a smaže je dalším příkazem ve stejné transakci (`BaseRepository.SystemTx`). Mutační kontrola přitom odhalila **chybu z fáze 4**. Souběžně vložený živý run se smazal spolu s tenantem (`runs.tenant_id` kaskáduje) a hromadné mazání při souběžném insertu uživatele skončilo chybou 23503, tedy 500. Mapování 23503 na „tenant není prázdný" odpadlo, protože po zámku 23503 nastat nemůže.
- **N16: ztracený zápis při editaci uživatele** (nový nález, sekce 2). `Update` a `UpdateAcrossTenants` zapisují jen sloupce, které edit vlastní: nikdy `active`, `password_hash` jen s novým heslem. Zámek řádku tak není potřeba a nezávisí na tom, jestli si na něj volající vzpomene. Změna hesla měla stejnou chybu v jednom sloupci: staré heslo ověřila proti přečtenému hashi a nové zapsala bez podmínky, takže přepsala reset hesla od admina. `UpdatePassword` je proto compare-and-swap (CAS, porovnej a zapiš) na ověřený hash. Rotace tokenu je CAS už dřív, jazyk nezávisí na přečtené hodnotě, runy hlídá owner-fencing a tenanti editaci nemají.
- **Zámek na kotvě nevznikl.** Žádný invariant přes víc řádků dnes neexistuje. Guardy superadmina kontroluje sám příkaz (`role <> 'superadmin'`) a unikátnost drží constraint. Vzor „nejdřív zamknout řádek, pak rozhodnout" používá mazání tenanta a popisuje ho `/gk-repositories`.
- **Hromadné zápisy uživatelů** zamykají řádky v pořadí podle id (`postgres.LockInIDOrder`). Deadlock dvou hromadných zápisů závisí na plánu dotazu a v testu nejde spolehlivě vyvolat, takže vlastní test nemají. Deadlock, který přesto nastane, vyřeší opakování.
- **PG dvojče `TestManager_ConcurrentTxWritesDoNotReturnBusy` nevzniklo.** Doslovný přepis (přečíst čítač, počkat, zapsat) by pod READ COMMITTED ztrácel zápisy z principu. S `FOR UPDATE` by zase ověřoval jen to, že fungují zámky řádků. Souběžné commandy bez ztráty zápisu hlídají testy editace uživatele (`app/application/user/command/concurrent_edit_test.go` a platformní dvojče), test změny hesla proti resetu (`app/application/profile/command/concurrent_change_test.go`) a test opakování po deadlocku přes produkční bus (`app/infrastructure/di/bus_retry_test.go`).
- **Testy dvou procesů.** `testfx.Fixture.Replica` otevře databázi fixture podruhé, jako druhá replika. `Fixture.RaceEdit` pustí souběžný zápis mezi čtení a zápis editu nebo změny hesla. Test scheduleru nad databází (`app/infrastructure/scheduler/replicas_db_test.go`) je v `excludeFiles` v `.go-arch-lint.yml`, stejně jako ostatní integrační testy nad testfx.
- **Ověřeno:** PostgreSQL 18.3 lokálně (OrbStack, služba `db-test`) i SQLite, celá suite s `-count=1`. Testy souběhu běžely na Postgresu 20× po sobě s `-race`. Šlo o claim runů, checkpoint, souběžné migrace, locker, dvě repliky scheduleru, deadlock přes bus, mazání tenanta proti insertu, editace proti deaktivaci, změna hesla proti resetu a rotace refresh tokenu. Každý nový test má mutační kontrolu.
