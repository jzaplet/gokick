...
- [x] **① Codegen (`gk tsgen`)** — Go DTO → TS, drift = fail v `make lint`. *(hotovo — dnes 22 anotovaných typů → 21 souborů, generováno a gate-ováno; několik request DTO schválně sdílí jeden TS typ. `healthResponse` je z tsgen vědomě vyňatý — infra-only endpoint, FE typ by byl dead code)*
