...
- [x] **④ Parita chybových polí (F-077)** — vyřešeno **statikou, bez golden testů** (Jiřího volba): `gk errfields` (v `make lint` jako `errfields-check`) extrahuje všechny Go `ValidationError{Field: "…"}` literály a klíče všech FE `*Errors` typů a kontroluje obousměrně — field bez FE domova i fantomový FE klíč = fail; `general` je konvenční catch-all (Field `""` + syntetizovaná selhání); dynamický (ne-literálový) `Field` = fail. Escape `//gkerrf:exempt <důvod>` (povinný důvod) pro fieldy, které nikdy nerenderují ve formuláři — dnes 8× `id` (path-param lookupy → redirect/toast) a tenant `name` (CLI-only). Kontrola je záměrně globální, ne per-endpoint (to by chtělo call-graph handler→command→VO — možné pozdější zpřesnění); překlepy a přejmenování — reálnou třídu driftu — chytá spolehlivě. Zero-sites guardy na obou stranách.
