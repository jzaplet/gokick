...
- [x] **③ FE typovaný fetch + lint** — `authFetch<Res, Err, Req>` / `apiFetch<Res, Err, Req>`: `FetchOptions<TBody = never>` znamená, že **body bez deklarovaného `Req` typu se nezkompiluje** (nic není přiřaditelné do `never`); `Req` se deklaruje generovaným request typem (`UserFormData`, `LoginRequest`, …), takže payload je strukturálně přibitý na Go DTO. ESLint (`no-restricted-syntax`, scoped na `assets/`) k tomu zavírá inference mezeru (volání bez explicitních generik) a zakazuje inline `body` literály (payload teče z typované proměnné). Ověřeno kousnutím všech tří vrstev. Výsledek: **fetch nejde poslat mimo generovaný typ.** A zpáteční směr taky: tsgen ke každému response typu generuje **runtime guard** (`is<Typ>`, opt-out `noguard` pro request-only DTO) a data-endpointy musí předat `validate:` (vynuceno typem — bez guardu se volání nezkompiluje, guard↔generika je párovaná přes `Guard<TData>`); 2xx tělo porušující kontrakt = transport failure + Sentry report. Syntetické chyby (network error, rozbité tělo, porušený kontrakt) přestaly předstírat `TError` — jsou samostatná union varianta `ApiTransport` a konzument bez zúžení (`isTransport`) na `data` nesáhne. Upload/download stojí ke smyčce takhle: `apiUpload` posílá multipart `FormData` (soubory — žádný JSON DTO, request půlka je z principu mimo tsgen; kontrakt jmen polí se u budoucího endpointu přibije golden testem), ale jeho **JSON response v paritě je** — `TData` = generovaný typ a explicit-generics ESLint pravidlo ho kryje; `apiDownload` je binární blob + `Content-Disposition` (žádné JSON na obou stranách, parita se netýká, `DownloadResult` je správně FE-only typ).
...
- [ ] **⑥ no-as ratchet (F-085)** — zapnout `@typescript-eslint/consistent-type-assertions` (zákaz `as`, výjimka `as const`) — runtime validace response hranice srazila inventář castů na **3**: `parseResponse.ts` `json as TError` (deklarované error tělo — odstraní ④), `parseResponse.ts` `null as TData` (invariant „bez guardu ⇔ TData=null" držený overloady apiFetch — kandidát na cílenou výjimku s důvodem, jako raw-pool exempts) a `useToast.ts` `JSON.parse(...) as` (localStorage hranice — potřebuje vlastní mini-guard). Po ④ + toast-guardu lze gate zapnout.
