...
- [ ] **⑤ Role union přes codegen (F-095)** — `AuthUser.role` / `UserFormData.role` jsou dnes codegen `string`; FE má ruční `Role` enum (`assets/app/Auth/enums/roles.ts`). Cíl: tsgen roli emituje sám, takže enum přestane být ručně udržovanou kopií Go konstant. **Průzkum hotový (2026-07-17), tohle je zadání:**

  - **Enum NEzmizí, jen se začne generovat.** Union type dá typ, ne hodnoty — a `Role.Admin` se používá jako **hodnota** na 8+ místech (`[Role.Admin]: '/admin/dashboard'`, `initial.role ?? Role.User`, `{ value: Role.User, label: 'User' }`). tsgen proto musí emitovat **trojici**: const objekt + `type` + guard, tj. přesně tvar dnešního `roles.ts`. Cesta souboru zůstane, takže 7 importérů se nedotkne.
  - **Hodnoty rolí žijí v `shared`, ne v `user`.** `user.Role` konsty jsou **konverze** (`RoleSuperAdmin = Role(shared.RoleSuperAdmin)`) — schválně, aby žebřík `IsPermissionAllowedForRole` a VO sdílely jednu definici. tsgen je AST-only, takže musí umět **dohledat konstantu přes balíčky** (`Role(shared.RoleSuperAdmin)` → `"superadmin"`). Zkratka „přečti literál lokálně" neexistuje — každá by ty stringy zduplikovala a zabila smysl jednoho zdroje pravdy. Tohle je nejrizikovější kus → chce vlastní fixture v `tools/gk/tsgen/testdata/`, která ten `type X string` + `X(other.Const)` vzor reprodukuje.
  - **`mapType` musí umět `*ast.SelectorExpr`** (`user.Role`) — dnes zvládá jen `*ast.Ident`, takže by na kvalifikovaném typu spadl.
  - **Guard generuj bez castu** (`v === Role.Admin || …`, NE `Object.values(Role) as string[]`) — jinak generovaný „DO NOT EDIT" soubor neprojde no-as ráčnou (⑥).
  - **Přetypovat jen 5 z 10 `Role` polí.** Skutečná role: `adminUserDTO`, `platformUserDTO`, `userDTO` (odpovědi) + `createUserRequest`/`updateUserRequest` a `platformUserRequest` (requesty). **Zbylé 4 musí zůstat `string`** — `bulkDeleteUsersRequest`, `bulkActiveUsersRequest` a jejich platformní dvojčata nesou **filtr z gridu**, kde `""` = nefiltrovat (sedí vedle `Active string`, taky filtr). Plošné přejmenování je rozbije.
  - **Přísnější guard je bezpečný, ověřeno.** Po ⑤ bude `isAdminUser` odmítat neznámou roli (dnes projde jakýkoli string) → `{ general }` + Sentry. To je korektní jen proto, že roli garantuje **DB**: `CHECK (role IN ('superadmin','admin','user'))` v `migrations/20260327000001_init_schema.sql`. Pozor, entita `user.User.Role` je `string`, ne VO, a čtecí cesta ji sqlx skenuje bez validace — bez toho CHECKu by tenhle krok vyrobil živou rejection cestu.
  - **`sonarjs/max-union-size` proto nezapínat** (viz FE lint gaty níže) — trestá přesně tenhle vzor.
