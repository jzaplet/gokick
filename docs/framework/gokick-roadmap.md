...
- [ ] **⑥ no-as ratchet (F-085)** — zapnout `@typescript-eslint/consistent-type-assertions` (zákaz `as`, výjimka `as const`) — runtime validace response hranice srazila inventář castů na **3**: `parseResponse.ts` `json as TError` (deklarované error tělo — od review kryté aspoň `isRecord` checkem, takže primitivum se za TError nevydává; plné odstranění dá ④), `parseResponse.ts` `null as TData` (invariant „bez guardu ⇔ TData=null" držený overloady apiFetch — kandidát na cílenou výjimku s důvodem, jako raw-pool exempts) a `useToast.ts` `JSON.parse(...) as` (localStorage hranice — potřebuje vlastní mini-guard). Po ④ + toast-guardu lze gate zapnout.
