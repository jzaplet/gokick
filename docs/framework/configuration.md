...
| `DOCUMAN_HTTP_PORT` | `3005` | Interní port, na kterém `documan` Docker service naslouchá; `docker-compose.yml` ho přes `${DOCUMAN_HTTP_PORT}` předá OrbStacku (`dev.orbstack.http-port`), který docs servíruje na **`https://docs.$APP_DOMAIN`** (bez host port mappingu, doména z `APP_DOMAIN`). Nesouvisí s aplikační binárkou. |
