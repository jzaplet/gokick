---
layout: 'page'
uri: '/framework/fire-and-forget'
position: 20
slug: 'framework-fire-and-forget'
parent: 'framework-background'
navTitle: 'Fire-and-forget'
...
**Fire-and-forget run** je krátký, „udělej a zapomeň" tvar [durable enginu](/framework/durable-run): práce,
...
> ℹ️ Fire-and-forget run a [durable run](/framework/durable-run) jsou **jeden engine** (tabulka `runs`, jeden worker,
> stejné lease/heartbeat). Liší se jen registrací: fire-and-forget run = `FireAndForget` (bez checkpointu),
> durable run = `Durable` (checkpoint + resume). Fire-and-forget run je run bez deníku. Kdy co → [Background work](/framework/overview).
...
resumovat** dosavadní postup (na to je [durable run](/framework/durable-run)). Zápis do fronty uvnitř
...
- [Durable run](/framework/durable-run) — durable tvar téhož enginu (checkpoint + resume po pádu).
- [Command](/framework/command) — transakce, ke které se zařazení do fronty připojí.
- [Events](/framework/events) — odkud se často zařazuje navazující práce.
