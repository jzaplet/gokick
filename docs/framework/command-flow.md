...
6. **RunDispatcher** — vloží `RunDispatcher` do `ctx` (handler může zařazovat joby/runy do fronty).
...
Dvě věci stojí za zapamatování: **Audit je mimo transakci** (bezpečnostní záznam přežije rollback, zapisuje se přes raw pool) a **DispatchEvents obaluje transakci** — eventy se rozešlou až po commitu, při rollbacku se zahodí. `Enqueue` jobu/runu naopak běží uvnitř transakce: business zápis i zařazení do fronty se uloží atomicky (samotný handler pak běží mimo transakci).
