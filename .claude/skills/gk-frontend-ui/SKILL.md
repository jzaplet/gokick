...
- **Maximální přísnost — tohle ti lint/tsc zařízne:** `interface` pro běžné typy (jen `type`; `interface` výjimečně u module augmentation, viz `router/meta.ts`), `obj.key` u index signatury (musí `obj['key']`), implicitní ne-boolean podmínka (`strict-boolean-expressions`), `==`/`!=` (jen `===`/`!==`), `console.*`/`debugger`/`alert`, soubor přes 300 řádků / vnoření hlubší než 4 (`max-lines`/`max-depth`), **kognitivní komplexita nad 15** (`sonarjs/cognitive-complexity` — kolik větvení a vnoření musíš udržet v hlavě; NE cyklomatická, ta trestá i ploché validační řetězce), a **`as` cast** (`consistent-type-assertions` — `as const` je z toho vyňatý, takže `Role`/`Permission` enumy platí dál). Na cast bez alternativy použij `eslint-disable-next-line` **s důvodem** (jako raw-pool výjimky v Go); dnes ho nesou jen dva generické casty v `parseResponse.ts`. **Konvence z `CLAUDE.md`, které lint nehlídá:** `function` keyword a `class` (jen arrow funkce / composables), `.forEach()` (použij `for...of`), `if (!x)` → `if (x === false)` / `if (x === null)`.
