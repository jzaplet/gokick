...
4. **Repo** (`app/infrastructure/sqlite/user/list.go`) — `FindPage`: tenant-scoped base + LIKE filtry (`listFilterWhere`), sort přes **mapu `listSortSQL`** (mapa JE injection guard — wire hodnota se nikdy neinterpoluje; textové sloupce řadí **česky** přes `database.CollateSort`; Postgres dvojče v `app/infrastructure/postgres/user/list.go`), povinný **tie-break `, id ASC`**, `LIMIT/OFFSET`; vrací `ListPage{ Items, Total }`, kde `Total` je **filtered count ze stejného WHERE** (jeden snapshot pro pager).
...
- **Textové sloupce řadí česky** — hodnota ve sort mapě končí `database.CollateSort` (`"nickname" + database.CollateSort`), stejně tak textový tie-break. Pořadí je abecední podle české abecedy (ch za h, č za c, malá před velkými), ne podle bajtů — a totéž dá i Postgres adaptér (ICU `cs-CZ`; hlídají zlaté testy `collation_test.go` v obou adaptérech). Id, čísla a časy řadí přirozeně, bez collation.
- **Textové filtry přes `database.LikeContains` + `database.LikeEscape`** (na Postgresu `postgres.ILikeContains`) — hledání je podřetězec bez ohledu na velikost písmen i u Č/Ř/Ž (Unicode `LIKE`) a to, co uživatel napsal, se hledá doslova: `%` a `_` nejsou wildcardy.
