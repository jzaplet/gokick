...
**Selhání vždy jednořádkově merguješ** — `data` na failure je `TErrors | { general: ApiMessage }`: chybové tělo z API, když přišlo (hodnoty jsou `ApiMessage` — `{key, params}`, ne věty), jinak syntetizované `{ general: ApiMessage }` s `fetch.*` klíčem (`{ key: 'fetch.network_error' }` pro síť, `fetch.malformed_body` / `fetch.invalid_shape` / `fetch.error_status` pro rozbité tělo či porušený kontrakt — stejný klíč `general`, kterým BE posílá ne-field chyby). Každý `*Errors` typ má `general?: ApiMessage`, takže `errors.value = result.data;` funguje bez zužování; zobrazení dělá `tm()` (`/gk-i18n`), takže i network error se vyrenderuje v aktivním jazyce v general slotu formuláře.
...
```
`parseResponse` (`Fetch/parseResponse.ts`) staví union podle `response.ok`. Všechna selhání bez použitelného chybového JSON objektu syntetizuje jako `{ general: ApiMessage }` (helper `generalFailure`, sdílený i apiFetchCore/apiUpload): 2xx s nenaparsovatelným tělem (`fetch.malformed_body`), 2xx porušující guard (`fetch.invalid_shape` + Sentry, URL v reportu normalizované — UUID → `:id`, ať se jeden bug negrupuje na issue per entita), chybový status s prázdným/ne-objektovým tělem (`fetch.error_status` s param `status`; holý JSON string od proxy se použije jako klíč — `tm()` ho zobrazí jako raw-key fallback) i síťová chyba (`fetch.network_error`, `status: 0`, nikdy výjimka). Chybové tělo se předá jako `TError` jen když je to skutečný JSON objekt (`isRecord`). 2xx s prázdným tělem bez guardu je success s `data: null`. Default `TError` je `ApiGeneralError` (`{ general: ApiMessage }`) — jediný tvar, který selhání reálně produkují; oba fasády sdílí jeden kontrakt `TypedFetchFn` (`Fetch/types/TypedFetchFn.ts`), takže se nemůžou rozjet. `apiFetchCore` (volná implementace pod fasádami) je pro aplikační kód zakázaný import (ESLint `no-restricted-imports`) — obcházel by celou validate↔TData disciplínu.
