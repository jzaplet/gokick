...
- `SkipPermissionCheck()` (interface `SkipPermission`) — výslovný opt-out pro veřejné operace, např. `login.go:19` / `refresh_token.go:19`.
...
2. **Business pravidla (I/O)** přes repo — unikátnost nicku (`h.users.FindByNickname`), existence (`h.users.FindByID`).

**Vedlejší efekty** se nesbírají přímo, ale přes per-request collectory z ctx:
- `shared.AuditCollectorFromContext(ctx).Record(...)` — pro security-relevant mutace (`user.created`, `user.role_changed` v `update_user.go:102`, `user.deleted`).
- `shared.EventCollectorFromContext(ctx).Collect(...)` — doménový event (`user.UserCreated` v `create_user.go:77`), na který reaguje někdo jiný. Detail: `/gk-domain-events`.

**Dispatch z HTTP handleru** (detail v `/gk-bus`):
- bez návratové hodnoty → `bus.ExecVoid(...)`.
- s typovaným výsledkem → `bus.Exec[T](...)`, např. `Login` vrací `LoginResult` (`handler/auth.go:90`).
...
7. Zaregistruj v DI + napiš HTTP handler, který pošle command přes `bus.ExecVoid` / `bus.Exec` (celý end-to-end checklist: `/gk-feature`).
